...
static bool aColl = false;
static bool aColl2 = false;
...
DWORD WINAPI UpdateThread(LPVOID)
{
	try
	{
		...
		while (true)
		{
			...
			if ((*Variables::m_UWorld) != nullptr && SDK::FName::GNames != nullptr)
			{
				...
				if (m_PlayerController != nullptr)
				{
					...
					if (m_PlayerController->AcknowledgedPawn != nullptr)
					{
						...
						if (GetAsyncKeyState('C') & 0x8000) {
							...
							{
								SDK::AActor* stuff = Variables::m_persistentLevel->AActors[i];

								if (stuff != nullptr)
								{
									if (stuff->IsA(SDK::AB_Pickups_C::StaticClass())) {
										auto pickup = static_cast<SDK::AB_Pickups_C*>(stuff);
										if (pickup->ItemDefinition ) {
											Variables::m_LocalPlayer->PlayerController->AcknowledgedPawn->K2_SetActorRelativeLocation(stuff->K2_GetActorLocation(), 0, true, nullptr);
											INPUT in;
						...
						if (GetAsyncKeyState('E') & 0x8000) {
							if (closestPlayer != nullptr)
							{
								...
								SDK::FVector playerLoc = closestPlayer->K2_GetActorLocation();
								SDK::FRotator playerRot = closestPlayer->K2_GetActorRotation();
								playerLoc.X -= 20;
								playerLoc.Y -= 20;
								if (!bat) { // aColl
									aColl = Variables::m_LocalPlayer->PlayerController->AcknowledgedPawn->bActorEnableCollision;
									Variables::m_LocalPlayer->PlayerController->AcknowledgedPawn->SetActorEnableCollision(false);
									Variables::m_LocalPlayer->PlayerController->AcknowledgedPawn->K2_SetActorRelativeLocation(playerLoc, 0, true, nullptr);
									Variables::m_LocalPlayer->PlayerController->AcknowledgedPawn->K2_SetActorRelativeRotation(playerRot, 0, true, nullptr);
									mouse_event(MOUSEEVENTF_LEFTDOWN, 0, 0, 0, 0);
									Sleep(100);
									mouse_event(MOUSEEVENTF_LEFTUP, 0, 0, 0, 0);
									Variables::m_LocalPlayer->PlayerController->AcknowledgedPawn->SetActorEnableCollision(aColl);
									bat = true;
								}
						...
						if (GetAsyncKeyState(0x02) & 0x8000){
							if (closestPlayer != nullptr)
							{
								...
								Utils::Engine::GetBoneLocation(static_cast<SDK::ACharacter*>(closestPlayer)->Mesh, &playerLoc, 66/*66*/);
...
HRESULT __stdcall hookD3D11Present(IDXGISwapChain* pSwapChain, UINT SyncInterval, UINT Flags)
{
	...
	//shaders
	if (!psRed)
		GenerateShader(pDevice, &psRed, 0.226f, 0.70f, 0.169f);

	if (!psGreen)
		GenerateShader(pDevice, &psGreen, 0.70f, 0.169f, 0.226f);
					...
					if (m_PlayerController->AcknowledgedPawn != nullptr)
					{
						...
						for (int i = 0; i < actors.Num(); i++)
						{
							...
							if (m_Player != nullptr)
							{
								...
								{
									if (m_Player->RootComponent != nullptr && !Utils::IsLocalPlayer(m_Player))
									{
										SDK::FVector playerLoc;
										Utils::Engine::GetBoneLocation(static_cast<SDK::ACharacter*>(m_Player)->Mesh, &playerLoc, 68);

										SDK::FVector2D Screen;
										bool FOV = Utils::Engine::WorldToScreen(Variables::m_LocalPlayer->PlayerController, m_Player->RootComponent->Location, &Screen);
										SDK::AFortPawn* AfortPlayer = static_cast<SDK::AFortPawn*>(actors[i]);
										if (FOV && pContext != nullptr && AfortPlayer->CurrentWeapon != nullptr)
										{
											
											if (AfortPlayer != nullptr)
											{
												SDK::FText weapon = AfortPlayer->CurrentWeapon->WeaponData->DisplayName;
												SDK::AFortPlayerStateAthena* playerstate = static_cast<SDK::AFortPlayerStateAthena*>(AfortPlayer->PlayerState);
												if (playerstate->PlayerName.IsValid() && strstr(AfortPlayer->GetName().c_str(), "PlayerPawn_Athena_C") && SDK::FName::GNames != nullptr) { // playerstate->CurrentHealth
													static wchar_t pop[30], pop2[30],pop3[30];
													swprintf_s(pop, L"HP:%2.2f", playerstate->CurrentHealth);
													swprintf_s(pop2, L"SH:%2.2f", playerstate->CurrentShield);
													if (weapon.Data->Name != nullptr)
													{
														swprintf_s(pop3, L"W:%s", weapon.Data->Name);
													}
													
													
														pFontWrapper->DrawString(pContext, playerstate->PlayerName.c_str(), 12, Screen.X, Screen.Y, 0xfff98d62, FW1_RESTORESTATE);
														pFontWrapper->DrawString(pContext, pop, 12, Screen.X, Screen.Y + 14, 0xfff98d62, FW1_RESTORESTATE);
														pFontWrapper->DrawString(pContext, pop2, 12, Screen.X, Screen.Y + (14 * 2), 0xfff98d62, FW1_RESTORESTATE);
														pFontWrapper->DrawString(pContext, pop3, 12, Screen.X, Screen.Y + (14 * 3), 0xfff98d62, FW1_RESTORESTATE);
														//printf("DBGDBGDBG");
